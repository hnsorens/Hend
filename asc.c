...
void decl_resolve(struct Decl *d) {
    ...
    {
        d->symbol = symbol_create(kind, d->type->subtype, d->name, 0, 0);
    }
    else
    {
...
typedef enum
{
    ...
    DAG_TYPE_OBJECT,
    DAG_TYPE_EXTEND,
    DAG_TYPE_REQUIREMENT,
    DAG_TYPE_INCLUDE,
    DAG_TYPE_CONSTRUCTOR,
    DAG_TYPE_INTEGER_8,
    DAG_TYPE_INTEGER_16,
    DAG_TYPE_INTEGER_32,
    DAG_TYPE_INTEGER_64,
    DAG_TYPE_UNSIGNED_INTEGER_8,
    DAG_TYPE_UNSIGNED_INTEGER_16,
    DAG_TYPE_UNSIGNED_INTEGER_32,
    DAG_TYPE_UNSIGNED_INTEGER_64,
    DAG_TYPE_INTEGER,
    DAG_TYPE_BOOLEAN,
    DAG_TYPE_CHARACTER,
    DAG_TYPE_STRING,
...
struct dag_node * dag_node_create_decl(struct Decl * d);
struct dag_node * dag_node_create_for(struct Stmt * s);
struct dag_node * dag_node_create_if(struct Stmt * s);
struct dag_node * dag_node_create_while(struct Stmt * s);
struct dag_node * dag_node_create_stmt(struct Stmt * s);
struct dag_node * dag_node_create_expr(struct Expr * e);
struct dag_node * dag_node_create_type(struct Type * t);
struct dag_node * dag_node_create_name(const char* name);
struct dag_node* dag_node_create_param(struct Param* param);
struct dag_node * dag_node_create_params(struct Param * p);
struct dag_node * dag_node_create_function_body(struct Decl * d);
struct dag_node * dag_node_create_function(struct Decl * d);
struct dag_node * dag_node_create_variable(struct Decl * d);
struct dag_node * dag_node_create_call(struct Stmt * s);
struct dag_node * dag_node_create_arguments(struct Expr * e);
struct dag_node * dag_node_create_argument(struct Expr * e);


struct dag_node * dag_node_create_assign(struct Decl * d)
{
    if (!d) return 0;

    struct dag_node * n = malloc(sizeof(*n));
    n->kind = DAG_DECL;
    n->left = dag_node_create_variable(d);
    n->right = dag_node_create_expr(d->value);
...
struct dag_node * dag_node_create_variable(struct Decl * d)
{
    ...
    struct dag_node * n = malloc(sizeof(*n));
    n->kind = DAG_VARIABLE;
    n->left = dag_node_create_type(d->type);
    n->right = dag_node_create_name(d->name);
...
struct dag_node * dag_node_create_decl(struct Decl * d)
{
    ...
    {
    case TYPE_FUNCTION:
        n->left = dag_node_create_function(d);
        n->right = dag_node_create_decl(d->next);
        break;
    default:
        n->left = dag_node_create_assign(d);
        n->right = dag_node_create_decl(d->next);
        break;
    }
...
struct dag_node * dag_node_create_for(struct Stmt * s)
{
    ...
    struct dag_node * next_e = malloc(sizeof(*next_e));
    next_e->kind = DAG_FOR_NEXT_EXPR;
    next_e->left = s->expr;
    next_e->right = s->next_expr;

    struct dag_node * init_e = malloc(sizeof(*init_e));
    init_e->kind = DAG_FOR_INIT_EXPR;
    init_e->left = next_e;
    init_e->right = s->init_expr;

    struct dag_node * n = malloc(sizeof(*n));
    n->kind = DAG_FOR;
    n->left = init_e;
    n->right = dag_node_create_stmt(s->body);
...
struct dag_node * dag_node_create_if(struct Stmt * s)
{
    ...
    struct dag_node * n = malloc(sizeof(*n));
    n->kind = DAG_IF;
    n->left = dag_node_create_expr(s->expr);
    n->right = dag_node_create_stmt(s->body);
...
struct dag_node * dag_node_create_argument(struct Expr * e)
{
    ...
    struct dag_node* n = malloc(sizeof(*n));
    // param_node->kind = DAG_PARAM;
    // param_node->left = dag_node_create_name(e->name); // Parameter name
    // param_node->right = dag_node_create_type(e->type); // Parameter type (assuming you have a type DAG node)
...
struct dag_node * dag_node_create_arguments(struct Expr * e)
{
    ...
    struct dag_node * arg_node = 0;
    struct dag_node * current_arg_node = 0;

    while (e != 0) {
        struct dag_node* param_node = dag_node_create_argument(e);
        if (arg_node == 0) {
            arg_node = arg_node;
            current_arg_node = arg_node;
        } else {
            current_arg_node->right = arg_node;
            current_arg_node = arg_node;
        }
        e = e->left;
    }
...
struct dag_node * dag_node_create_stmt(struct Stmt * s)
{
    ...
    case STMT_BLOCK:
        break;
    case STMT_EXPR:
        n = dag_node_create_expr(s);
        break;
    case STMT_CALL:
        n = dag_node_create_call(s);
        break;
    case STMT_DECL:
        n =  dag_node_create_decl(s->decl);
        break;
    case STMT_FOR:
        n = dag_node_create_for(s);
        break;
    case STMT_IF:
        n = dag_node_create_if(s);
        break;
    case STMT_RETURN:
        n->kind = DAG_RETURN;
        n->left = dag_node_create_expr(s->expr);
...
struct dag_node * dag_node_create_expr(struct Expr * e)
{
    ...
    switch (e->kind) {
        case EXPR_ASSIGN:
            n->kind = DAG_ASSIGN;
            n->left = dag_node_create_name(e->left->name);
            n->right = dag_node_create_expr(e->right);
            break;
        case EXPR_NAME:
            n = dag_node_create_name(e->name);
            break;
        case EXPR_INTEGER:
            n->kind = DAG_INTEGER_VALUE;
            n->u.integer_value = e->integer_value;
            break;
        case EXPR_BOOLEAN:
            n->kind = DAG_BOOLEAN_VALUE;
            n->u.integer_value = e->integer_value;
            break;
        case EXPR_ADD:
            if (is_num(e->left) && is_num(e->right))
            {
                n->kind = DAG_INTEGER_VALUE;
                n->u.integer_value = e->left->integer_value + e->right->integer_value;
            }
            else
            {
                n->kind = DAG_IADD;
                n->left = dag_node_create_expr(e->left);
                n->right = dag_node_create_expr(e->right);
            }
            break;
        case EXPR_SUB:
        ...
        case EXPR_ARG:
            break;
        case EXPR_CHAR:
            n->u.character_value = e->char_literal;
            break;
        case EXPR_STRING:
            n->u.string = e->string_literal;
            break;
        default:
            break;
    }
...
struct dag_node* dag_node_create_param(struct Param* param) 
{
    ...
    struct dag_node* param_node = malloc(sizeof(*param_node));
    param_node->kind = DAG_PARAM;
    param_node->left = dag_node_create_name(param->name); // Parameter name
    param_node->right = dag_node_create_type(param->type); // Parameter type (assuming you have a type DAG node)
...
struct dag_node * dag_node_create_params(struct Param * p)
{
    ...
    struct dag_node * params_node = 0;
    struct dag_node * current_param_node = 0;

    while (p != 0) {
        struct dag_node* param_node = dag_node_create_param(p);
        if (params_node == 0) {
            params_node = param_node;
            current_param_node = param_node;
        } else {
            current_param_node->right = param_node;
            current_param_node = param_node;
        }
        p = p->next;
    }
...
struct dag_node * dag_node_create_function_body(struct Decl * d)
{
    ...
    struct dag_node * n = malloc(sizeof(*n));
    n->kind = DAG_BLOCK;
    n->left = dag_node_create_params(d->type->params);
    n->right = dag_node_create_stmt(d->code);
...
struct dag_node * dag_node_create_function(struct Decl * d)
{
    ...
    struct dag_node * n = malloc(sizeof(*n));
    n->kind = DAG_ASSIGN;
    n->left = dag_node_create_name(d->name);
    n->right = dag_node_create_function_body(d);
